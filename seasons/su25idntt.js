...
async function loadCards() {
    try {
        const response = await fetch('json/su25idntt.json'); // Load from JSON file
...
function getCombination() {
    ...
    if (rand < 0.60) {
	    return { basic: 14, special: 0, unit: 0, fail: 2 };
    } else if (rand < 0.90) {
        return { basic: 14, special: 1, unit: 0, fail: 1 }; 
    } else if (rand < 0.99) {
        return { basic: 14, special: 1, unit: 1, fail: 0 };
    } else {
        return { basic: 14, special: 0, unit: 1, fail: 1 };
    }
}
