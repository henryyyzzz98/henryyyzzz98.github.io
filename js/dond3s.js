...
function startGame() {
  ...
  addLog({
    ...
    prizeBoard: [...prizes],
    caseDistribution: {
      "UP 1": 14, "UP 2": 4, "UP 3": 2, "DOWN 1": 3, "STRIKE": 3
    }
  });
...
function renderCases() {
  ...
  cases.forEach(gameCase => {
    ...
    if (gameCase.opened) {
      button.classList.add("disabled", "revealed", resultClass(gameCase.result));
...
async function openCase(gameCase) {
  ...
  const button = document.querySelector(`.case[data-case-number="${gameCase.number}"]`);
  if (!button) { waitingForDeal = false; return; }
  ...
  if (openedCasesThisRound >= casesToOpen) {
    // Keep the board locked while the Banker offer is being prepared.
    showBanker();
  } else {
    // Re-enable case selection before rendering the cases.
    // This is important because renderCases only attaches click handlers
    // when waitingForDeal is false.
    waitingForDeal = false;
    renderCases();
...
function getResultMessage(result) {
  switch (result) {
    case "UP 1": return "UP 1! CLIMB ONE LEVEL.";
    case "UP 2": return "UP 2! CLIMB TWO LEVELS.";
    case "UP 3": return "UP 3! CLIMB THREE LEVELS.";
    case "DOWN 1": return "DOWN 1! DROP ONE LEVEL.";
    case "STRIKE": return "STRIKE! THE HIGHEST PRIZE IS STRUCK OUT.";
...
function applyResult(result) {
  ...
  if (result === "STRIKE") {
    ...
    if (currentPrizeIndex >= prizes.length) currentPrizeIndex = prizes.length - 1;
  }

  currentPrizeIndex = Math.max(0, Math.min(currentPrizeIndex, prizes.length - 1));
...
function updateGameInfo() {
  ...
  casesLeftElement.textContent = cases.filter(gameCase => !gameCase.opened).length;
  updateCurrentPrizeDisplay();
}

function getStrikesRemaining() {
  return cases.filter(gameCase => !gameCase.opened && gameCase.result === "STRIKE").length;
}
...
async function showBanker() {
  ...
  const offer = buyoutAvailable ? calculateBuyoutOffer(normalOffer) : normalOffer;
  ...
  instruction.textContent = buyoutAvailable ? "THE BANKER HAS A SPECIAL OFFER" : "THE BANKER IS CALLING...";
  message.textContent = buyoutAvailable ? "ONE STRIKE REMAINS. THE BANKER WANTS TO BUY YOU OUT." : "PLEASE WAIT FOR THE BANKER'S OFFER.";
  ...
  addLog({
    ...
    topPrize: prizes[prizes.length - 1],
    strikesRemaining: getStrikesRemaining(),
    unopenedCases: cases.filter(gameCase => !gameCase.opened).map(gameCase => ({
      caseNumber: gameCase.number,
  ...
  if (title) title.textContent = buyoutAvailable ? "BANKER BUYOUT" : "BANK OFFER";
  ...
  message.textContent = buyoutAvailable ? "TAKE THE BUYOUT OR KEEP CLIMBING." : "WILL YOU TAKE THE DEAL?";
}
...
function calculateBankerOffer() {
  ...
  const highestPrize = prizes[prizes.length - 1];
  const casesRemaining = cases.filter(gameCase => !gameCase.opened).length;

  let roundFactor;
  switch (round) {
    case 1: roundFactor = 0.3; break;
    case 2: roundFactor = 0.4; break;
    case 3: roundFactor = 0.5; break;
    case 4: roundFactor = 0.6; break;
    case 5: roundFactor = 0.7; break;
    case 6: roundFactor = 0.76; break;
    case 7: roundFactor = 0.8; break;
    case 8: roundFactor = 0.84; break;
    case 9: roundFactor = 0.9; break;
    default: roundFactor = 0.9;
  }

  const topPrizeRatio = highestPrize / expectedValue;
  let riskFactor;
  if (topPrizeRatio >= 10) riskFactor = 0.85;
  else if (topPrizeRatio >= 7) riskFactor = 0.9;
  else if (topPrizeRatio >= 5) riskFactor = 0.95;
  else if (topPrizeRatio >= 3) riskFactor = 1.0;
  else riskFactor = 1.05;

  let caseFactor;
  if (casesRemaining > 15) caseFactor = 0.9;
  else if (casesRemaining > 10) caseFactor = 0.95;
  else if (casesRemaining > 5) caseFactor = 1.0;
  else caseFactor = 1.05;

  let offer = expectedValue * roundFactor * riskFactor * caseFactor;
  offer *= 0.95 + Math.random() * 0.1;
  offer = Math.max(offer, expectedValue * 0.15);
  offer = Math.min(offer, expectedValue);
  return smartRoundOffer(offer);
}
...
function takeDeal() {
  ...
  message.textContent = buyout ? `YOU ACCEPTED THE BUYOUT: ${formatMoney(offer)}!` : `YOU ACCEPTED ${formatMoney(offer)}!`;
...
function generateGameLog() {
  ...
  output += "Case Distribution: 14 UP 1 | 4 UP 2 | 2 UP 3 | 3 DOWN 1 | 3 STRIKE\n\n";
  ...
  Object.keys(rounds).sort((a,b) => Number(a)-Number(b)).forEach(r => {
    ...
    rounds[r].filter(e => e.type === "CASE_OPENED").forEach(e => {
      output += `Case #${e.caseNumber} → ${e.result} | Prize before: ${formatMoney(e.prizeBefore)}\n`;
    });

    rounds[r].filter(e => e.type === "PRIZE_POSITION_CHANGED").forEach(e => {
    ...
    rounds[r].filter(e => e.type === "STRIKE").forEach(e => {
      output += `STRIKE: ${formatMoney(e.removedPrize)} removed | New Top Prize: ${formatMoney(e.newTopPrize)}\n`;
    });

    rounds[r].filter(e => e.type === "BANKER_OFFER" || e.type === "BANKER_BUYOUT").forEach(e => {
      output += `${e.type === "BANKER_BUYOUT" ? "BANKER BUYOUT" : "Banker's Offer"}: ${formatMoney(e.offer)}\n`;
      if (e.normalOffer != null && e.type === "BANKER_BUYOUT") output += `Normal Offer Before Buyout: ${formatMoney(e.normalOffer)}\n`;
      ...
      if (e.unopenedCases?.length) {
        ...
        e.unopenedCases.forEach(c => output += `Case #${c.caseNumber} → ${c.result}\n`);
      }
    });

    rounds[r].filter(e => ["DEAL","BUYOUT_ACCEPTED","BUYOUT_REJECTED","NO_DEAL"].includes(e.type)).forEach(e => {
      if (e.type === "DEAL") output += `Decision: DEAL → ${formatMoney(e.amount)}\n`;
      if (e.type === "BUYOUT_ACCEPTED") output += `Decision: BUYOUT ACCEPTED → ${formatMoney(e.amount)}\n`;
      if (e.type === "BUYOUT_REJECTED") output += "Decision: BUYOUT REJECTED / NO DEAL\n";
  ...
  if (end) {
    ...
    if (end.currentPrize != null) output += `Final Current Prize: ${formatMoney(end.currentPrize)}\n`;
    if (end.topPrize != null) output += `Final Top Prize: ${formatMoney(end.topPrize)}\n`;
  }
  output += "\n========================================\n              END OF GAME\n========================================\n";
  return output;
}
...
function downloadGameLog() {
  const blob = new Blob([generateGameLog()], { type: "text/plain;charset=utf-8" });
