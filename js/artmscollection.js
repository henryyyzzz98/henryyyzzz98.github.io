...
let selectedartmsCollections = JSON.parse(
  localStorage.getItem("selectedartmsCollections") || "[]",
);
...
async function loadCollections() {
  try {
    const response = await fetch("json/allartms.json");
...
function renderCollectionGroup(prefix, collectionList) {
  ...
  collectionList.forEach((collection) => {
    ...
    if (selectedartmsCollections.includes(collection)) {
      btn.classList.add("selected");
    }

    btn.addEventListener("click", () => {
      if (selectedartmsCollections.includes(collection)) {
        selectedartmsCollections = selectedartmsCollections.filter(
          (c) => c !== collection,
        );

        btn.classList.remove("selected");
      } else {
        selectedartmsCollections.push(collection);
        btn.classList.add("selected");
      }

      updateTabHighlights();
      updateselectedartmsCollectionsMessage();
    });
...
function updateselectedartmsCollectionsMessage() {
  const msg = document.getElementById("selectedartmsCollectionsMsg");

  if (!msg) return;

  if (!selectedartmsCollections.length) {
    msg.textContent = "No collections selected.";
    return;
  }

  msg.textContent = "Selected collections: " + selectedartmsCollections.join(", ");
}
...
function updateTabHighlights() {
  ...
  tabs.forEach((tab) => {
    ...
    const anySelected = selectedartmsCollections.some((collection) => {
      if (prefix === "AA") {
...
document.getElementById("saveCollectionBtn").addEventListener("click", () => {
  localStorage.setItem(
    "selectedartmsCollections",
    JSON.stringify(selectedartmsCollections),
  );
...
window.onload = async () => {
  ...
  if (selectedartmsCollections.length > 0) {
    generateRandomSet();
    displayCards();
  }
