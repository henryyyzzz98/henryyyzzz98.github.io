...
const ROUND_CASES = [3, 3, 2, 2, 1, 1, 1, 1];
const CASE_RESULTS = [
  ...Array(11).fill("UP 1"),
  ...Array(4).fill("STRIKE")
];
...
function getResultMessage(result) {
  switch (result) {
    ...
    case "STRIKE":
      return "STRIKE! MOVE BACK 1 STEP — TOP PRIZE DROPS!";
    default:
      return result;
  }
}
...
function applyResult(result) {
  ...
  if (result === "STRIKE") {
    ...
    const strikeNumber = 4 - strikesRemaining;

    // Fourth Strike = immediate game-ending loss
    if (strikeNumber >= 4) {
      addLog({
        type: "STRIKE",
        round,
    ...
    /*
      STRIKE EFFECT:
      1. Move contestant BACK 1 step.
      2. Remove the highest prize from the ladder.
    */

    // Move down 1 step
    currentPrizeIndex -= 1;

    // Remove the highest prize
    let removedPrize = null;
    let newTopPrize = null;

    if (prizes.length > 1) {
      removedPrize = prizes.pop();
...
function calculateBankerOffer() {
  ...
  */
  ...
  remainingCases.forEach((gameCase) => {
    ...
    // A Strike moves the contestant back 1 step
    // AND removes the highest prize.
    simulatedIndex -= 1;

    simulatedPrizes = prizes.slice(0, -1);

    simulatedIndex = Math.min(
        simulatedIndex,
        simulatedPrizes.length - 1
    );
    }
  ...
  Object.keys(rounds)
    ...
    .forEach((r) => {
      ...
      rounds[r]
        ...
        .forEach((e) => {
          if (e.strikeNumber === 4) {
            output += "STRIKE #4: GAME OVER — PLAYER WINS $0\n";
          } else {
